...
public class Number {
    ...
    public Number(NumberSystem numberSystem, String digit) {
        ...
        if (numberSystemIntValue == 10) {
            if (!digit.matches("^\\d+$")) throw new NumberFormatException();
        } else if (numberSystemIntValue == 12) {
            if (!digit.matches("^[^\\sc-zA-Z]+$")) throw new NumberFormatException();
        } else if (numberSystemIntValue == 16) {
            if (!digit.matches("^[^\\sg-zA-Z]+$")) throw new NumberFormatException();
        } else if (numberSystemIntValue > 1 && numberSystemIntValue < 10)
            if (!digit.matches("^[0-" + (numberSystemIntValue-1) + "\\d]+$")) throw new NumberFormatException();
